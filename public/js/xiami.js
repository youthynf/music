...
$(function(){
	...
	$(".start em").click(function(){
		...
		var s_lrc=$(this).attr("s_lrc")
		var tu=$(this).attr("son");
		var zhuan=$(this).attr("a_img");
		var song_resource=$(this).attr("song_resource");
		songIndex=sid;
		$("#audio").attr("src",song_resource);
		$("#canvas1").attr("src",tu);
		$("#canvas2").attr("src",zhuan);
...
{ 	
	...
	//};
	//return "[00:00] .[00:02]纯音乐暂无歌词盛夏光年（live）";	
 	return s_lrc;
}
...
function getReady(s_lrc)//在显示歌词前做好准备工作
{ 	
	var ly=getLy(s_lrc);//得到歌词
	//alert(ly);
	// lytext.length=0;
	// lytime.length=0;
	// lytext = new Array();
